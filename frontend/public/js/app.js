...
function navigate(p){
  ...
  const lk=document.querySelector(`[data-page="${p}"]`);
  if(lk)lk.classList.add('active');
  // Update mobile topbar page title
  const titles={dashboard:'Dashboard',challenges:'My Challenges',funded:'Funded Accounts',buy:'Buy Challenge',payouts:'Payouts',trades:'Trade History',coach:'AI Coach',rules:'Rules',affiliate:'Affiliate',settings:'Settings',certs:'Certificates',leaderboard:'Leaderboard',calendar:'Calendar'};
  const titleEl=$('mobilePageTitle');
...
$('page-challenges').innerHTML=`<div class="page-head"><h1>Account Metrics</h1><p>Detailed analytics for each evaluation</p></div>${d.map(c=>{const profit=c.current_balance-c.starting_balance;const profitPct=profit/c.starting_balance*100;const ddUsed=(c.highest_balance-c.lowest_equity)/c.starting_balance*100;const wr=c.total_trades?Math.round(c.winning_trades/c.total_trades*100):0;const isActive=c.status==='active';const isPassed=c.status==='passed';const isFailed=c.status==='failed';const isPending=c.status==='pending_payment';const phase=c.challenge_type==='two_step'?(c.phase===2
    ?' <span style="display:inline-flex;align-items:center;padding:2px 8px;background:rgba(251,191,36,.12);color:#fbbf24;border:1px solid rgba(251,191,36,.25);border-radius:10px;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-left:6px;vertical-align:middle">Phase 2</span>'
    :' <span style="display:inline-flex;align-items:center;padding:2px 8px;background:var(--acbg);color:var(--ac2);border:1px solid var(--acgl);border-radius:10px;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-left:6px;vertical-align:middle">Phase 1</span>'
  ):'';
...
<button id="buySubmitBtn" class="btn btn-primary btn-full" style="padding:16px;font-size:.95rem" onclick="submitChallengePurchase()">Continue to Payment</button>
...
async function submitChallengePurchase(){
  ...
  try{
    ...
    else{toast('Challenge activated! Open PlutoTrader to start trading.','success');navigate('challenges')}
  }catch(x){
