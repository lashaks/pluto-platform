...
const LOADING='<div style="text-align:center;padding:60px;color:var(--t3)"><div style="width:28px;height:28px;border:3px solid rgba(255,255,255,.06);border-top-color:var(--ac);border-radius:50%;animation:spin .6s linear infinite;margin:0 auto"></div></div>';
...
function initParticles(canvasId,opts={}){
  ...
  const count=opts.count||80,speed=opts.speed||0.3,maxDist=opts.maxDist||140,
        color=opts.color||'139,92,246',baseAlpha=opts.baseAlpha||0.4;
  ...
  function init(){
    ...
    for(let i=0;i<count;i++){
      particles.push({x:Math.random()*w,y:Math.random()*h,
        ...
        r:Math.random()*2+1,alpha:Math.random()*baseAlpha+0.1})
    }
  }
  function draw(){
    ...
    for(let i=0;i<particles.length;i++){
      for(let j=i+1;j<particles.length;j++){
        ...
        if(dist<maxDist){
          ...
          ctx.strokeStyle=`rgba(${color},${alpha})`;ctx.lineWidth=0.5;ctx.stroke();
        }
      }
      // Mouse interaction
      if(mouse.x!==null){
        ...
        if(dist<200){
          ...
          ctx.strokeStyle=`rgba(${color},${alpha})`;ctx.lineWidth=0.8;ctx.stroke();
          // Gentle push
...
function toast(m,t='info'){const e=document.createElement('div');e.className='toast toast-'+t;e.textContent=m;document.body.appendChild(e);setTimeout(()=>{e.style.opacity='0';e.style.transform='translateY(-10px)';setTimeout(()=>e.remove(),300)},3000)}
...
function toggleMobileSidebar(){$('sidebar').classList.toggle('open');$('mobileOverlay').classList.toggle('hidden')}
function closeMobile(){$('sidebar').classList.remove('open');$('mobileOverlay').classList.add('hidden')}

/* ======= PRICING ======= */
const PLANS={one_step:[{size:10000,fee:79,target:10,daily:5,dd:8,split:80,lev:'1:30'},{size:25000,fee:179,target:10,daily:5,dd:8,split:80,lev:'1:30'},{size:50000,fee:299,target:10,daily:5,dd:8,split:80,lev:'1:20'},{size:100000,fee:499,target:10,daily:5,dd:8,split:80,lev:'1:20'},{size:200000,fee:949,target:10,daily:5,dd:8,split:80,lev:'1:20'}],two_step:[{size:10000,fee:59,target:'8 / 5',daily:5,dd:8,split:80,lev:'1:30'},{size:25000,fee:139,target:'8 / 5',daily:5,dd:8,split:80,lev:'1:30'},{size:50000,fee:239,target:'8 / 5',daily:5,dd:8,split:80,lev:'1:20'},{size:100000,fee:399,target:'8 / 5',daily:5,dd:8,split:80,lev:'1:20'},{size:200000,fee:749,target:'8 / 5',daily:5,dd:8,split:80,lev:'1:20'}]};
function renderPricing(plans,container,action){container.innerHTML=plans.map((p,i)=>`<div class="plan ${i===3?'popular':''}" onclick="${action}(${p.size})"><div class="plan-size">${F(p.size).replace('.00','')}</div><div class="plan-price">${F(p.fee).replace('.00','')}</div><div class="plan-detail">${p.target}% Target</div><div class="plan-detail">${p.daily}% Daily Loss</div><div class="plan-detail">${p.dd}% Drawdown</div><div class="plan-detail">${p.split}% Split</div><div class="plan-detail">${p.lev}</div><button class="btn btn-primary btn-sm btn-full" style="margin-top:16px">${action==='selectPlan'?'Get Funded':'Select Plan'}</button></div>`).join('')}
...
/* ======= DASHBOARD ======= */
window.render_dashboard=async function(){$('page-dashboard').innerHTML=LOADING;try{const s=await api('/api/dashboard/stats');const pending=s.challenges.filter(c=>c.status==='pending_payment');const active=s.challenges.filter(c=>c.status==='active');$('page-dashboard').innerHTML=`<div class="page-head"><h1>Dashboard</h1><p>Welcome back, ${user.first_name}.</p></div><div class="stats"><div class="stat s-purple"><div class="stat-label">Total Profit</div><div class="stat-value">${F(s.total_profit)}</div></div><div class="stat s-green"><div class="stat-label">Total Payouts</div><div class="stat-value">${F(s.total_payouts)}</div></div><div class="stat s-blue"><div class="stat-label">Active Challenges</div><div class="stat-value">${s.active_challenges}</div></div><div class="stat s-cyan"><div class="stat-label">Funded Accounts</div><div class="stat-value">${s.active_funded}</div></div><div class="stat s-amber"><div class="stat-label">Win Rate</div><div class="stat-value">${s.win_rate}%</div></div><div class="stat s-green"><div class="stat-label">Total Trades</div><div class="stat-value">${s.total_trades}</div></div></div>${pending.length?`<div class="card" style="border-color:var(--am);background:var(--am-bg)"><div class="card-title" style="color:var(--am)">&#9888; Pending Payment</div>${pending.map(c=>`<div class="row"><span class="row-label">${F(c.account_size)} ${c.challenge_type==='two_step'?'2-Step':'1-Step'}</span><span class="row-value" style="color:var(--am)">Awaiting crypto payment</span></div>`).join('')}<p style="color:var(--t2);font-size:.82rem;margin-top:10px">Complete your crypto payment to activate your challenge.</p></div>`:''}${active.length?`<div class="card"><div class="card-title">Active Accounts</div>${active.map(c=>{const pp=((c.current_balance-c.starting_balance)/c.starting_balance*100);const tp=Math.min(100,Math.max(0,pp/c.profit_target_pct*100));const du=((c.highest_balance-c.lowest_equity)/c.starting_balance*100);const dp=Math.min(100,du/c.max_total_loss_pct*100);return`<div class="detail"><div class="detail-head"><div><div class="detail-title">${F(c.account_size)} Challenge</div><div class="detail-sub">Login: ${c.ctrader_login||'\u2014'} &bull; ${c.challenge_type==='two_step'?'2-Step':'1-Step'}</div></div>${B(c.status)}</div><div class="row"><span class="row-label">Profit Target (${c.profit_target_pct}%)</span><span class="row-value ${pp>=0?'pos':'neg'}">${pct(pp)} / ${c.profit_target_pct}%</span></div><div class="bar"><div class="bar-fill bar-green" style="width:${tp}%"></div></div><div class="row" style="margin-top:12px"><span class="row-label">Balance</span><span class="row-value">${F(c.current_balance)}</span></div><div class="row"><span class="row-label">Equity</span><span class="row-value">${F(c.current_equity)}</span></div><div class="row"><span class="row-label">Drawdown Used</span><span class="row-value" style="color:${dp>70?'var(--rd)':'var(--t1)'}">${du.toFixed(2)}% / ${c.max_total_loss_pct}%</span></div><div class="bar"><div class="bar-fill ${dp>70?'bar-red':'bar-purple'}" style="width:${dp}%"></div></div><div class="row" style="margin-top:12px"><span class="row-label">Trades</span><span class="row-value">${c.total_trades} (${c.winning_trades}W / ${c.losing_trades}L)</span></div><div class="row"><span class="row-label">Best Day</span><span class="row-value pos">${F(c.best_day_profit)}</span></div></div>`}).join('')}</div>`:`<div class="card"><div class="empty">No active challenges yet.<br><a onclick="navigate('buy')">Purchase your first challenge &rarr;</a></div></div>`}`}catch(e){$('page-dashboard').innerHTML='<div class="card"><div class="empty">Failed to load dashboard.</div></div>'}};
...
window.render_funded=async function(){$('page-funded').innerHTML=LOADING;const d=await api('/api/funded');$('page-funded').innerHTML=`<div class="page-head"><h1>Funded Accounts</h1><p>Your funded trading accounts</p></div>${d.map(a=>`<div class="detail"><div class="detail-head"><div class="detail-title">${F(a.account_size)} Funded</div>${B(a.status)}</div><div class="row"><span class="row-label">Balance</span><span class="row-value">${F(a.current_balance)}</span></div><div class="row"><span class="row-label">Total Profit</span><span class="row-value pos">${F(a.total_profit)}</span></div><div class="row"><span class="row-label">Profit Split</span><span class="row-value">${a.profit_split_pct}%</span></div><div class="row"><span class="row-label">Total Payouts</span><span class="row-value" style="color:var(--gr)">${F(a.total_payouts)}</span></div><div class="row"><span class="row-label">Scaling Level</span><span class="row-value">Level ${a.scaling_level}</span></div><button class="btn btn-primary btn-sm" style="margin-top:16px" onclick="showPayoutModal('${a.id}')">Request Payout</button></div>`).join('')||'<div class="card"><div class="empty">No funded accounts yet. Pass an evaluation to get funded.</div></div>'}`};

window.render_buy=async function(){$('page-buy').innerHTML=`<div class="page-head"><h1>Buy Challenge</h1><p>Choose your evaluation type and account size</p></div><div class="eval-tabs"><button class="eval-tab active" onclick="switchBuyEval('one_step',this)">1-Step Evaluation</button><button class="eval-tab" onclick="switchBuyEval('two_step',this)">2-Step Evaluation</button></div><div class="buy-grid" id="buyGrid"></div><div class="card" style="margin-top:18px"><div class="card-title">Included With Every Plan</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;color:var(--t2);font-size:.84rem"><div>&#10003; No time limit</div><div>&#10003; No minimum trading days</div><div>&#10003; Static drawdown</div><div>&#10003; EAs and copy trading</div><div>&#10003; Hedging permitted</div><div>&#10003; cTrader &amp; MetaTrader 5</div><div>&#10003; Crypto &amp; bank payouts</div><div>&#10003; Scale up to $2M</div><div>&#10003; 24/7 support</div></div></div>`;renderPricing(PLANS[currentEval],$('buyGrid'),'purchase')};
...
/* ======= INIT ======= */
function handleReturnFromPayment(){const p=new URLSearchParams(window.location.search);if(p.get('purchased')==='true'){toast('Payment received! Your challenge will be activated shortly.','success');window.history.replaceState({},'',window.location.pathname)}}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  initParticles('heroCanvas',{count:70,speed:0.25,maxDist:150,color:'139,92,246',baseAlpha:0.35});
  initParticles('ctaCanvas',{count:40,speed:0.2,maxDist:120,color:'139,92,246',baseAlpha:0.25});
});
