...
async function doForgot(e){e.preventDefault();try{pendingResetEmail=$('forgotEmail').value;await api('/api/auth/forgot-password',{method:'POST',body:JSON.stringify({email:pendingResetEmail})});showAuthScreen('formReset');toast('Check your email for a reset code.','info')}catch(x){toast(x.message,'error')}}
async function doReset(e){e.preventDefault();try{await api('/api/auth/reset-password',{method:'POST',body:JSON.stringify({email:pendingResetEmail,code:$('resetCode').value,new_password:$('resetNewPass').value})});toast('Password reset!','success');showAuth('login')}catch(x){toast(x.message,'error')}}
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:1px;background:var(--brd);border-radius:var(--r2);overflow:hidden;margin-bottom:20px"><div style="padding:14px 16px;background:var(--bg)"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:4px">Platform</div><div style="font-size:.92rem;font-weight:600">${c.platform==='mt5'?'MetaTrader 5':c.platform==='matchtrader'?'Match-Trader':'cTrader'}</div></div><div style="padding:14px 16px;background:var(--bg)"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:4px">Login</div><div style="font-size:.92rem;font-family:var(--fm);font-weight:700;color:var(--ac2)">${c.ctrader_login||'—'}</div></div><div style="padding:14px 16px;background:var(--bg)"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:4px">Password</div><div style="font-size:.92rem;font-family:var(--fm);color:var(--t1)">${c.ctrader_password||'—'}</div></div><div style="padding:14px 16px;background:var(--bg)"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:4px">Server</div><div style="font-size:.92rem;font-family:var(--fm)">${c.ctrader_server||'Demo'}</div></div><div style="padding:14px 16px;background:var(--bg)"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:4px">Leverage</div><div style="font-size:.92rem;font-weight:600">${c.leverage||'1:30'}</div></div></div>
<div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:20px">${G('Profit Target',profitPct,c.profit_target_pct,'var(--gr)',false)}${G('Daily Loss',0,c.max_daily_loss_pct,'var(--am)',true)}${G('Max Drawdown',ddUsed,c.max_total_loss_pct,'var(--ac2)',true)}</div>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:8px;margin-bottom:16px">${M('Balance',F(c.current_balance))}${M('Equity',F(c.current_equity))}${M('Profit',F(profit),profit>=0?'var(--gr)':'var(--rd)')}${M('Win Rate',wr+'%')}${M('Trades',c.total_trades)}${M('W / L',`<span style="color:var(--gr)">${c.winning_trades}</span><span style="color:var(--t3)"> / </span><span style="color:var(--rd)">${c.losing_trades}</span>`)}</div>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:6px;padding:14px;background:var(--bg);border-radius:var(--r2);border:1px solid var(--brd);font-size:.8rem"><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:${profitPct>=c.profit_target_pct?'var(--gr)':'var(--t3)'}">&#${profitPct>=c.profit_target_pct?'10004':'9679'};</span> Target ${c.profit_target_pct}%</div><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:${ddUsed<c.max_total_loss_pct?'var(--gr)':'var(--rd)'}">&#${ddUsed<c.max_total_loss_pct?'10004':'10006'};</span> DD ${c.max_total_loss_pct}%</div><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:var(--gr)">&#10004;</span> Daily ${c.max_daily_loss_pct}%</div><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:var(--t3)">&#9679;</span> 20% Consistency</div><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:var(--t3)">&#9679;</span> News Restricted</div><div style="display:flex;align-items:center;gap:6px;color:var(--t2)"><span style="color:var(--gr)">&#10004;</span> No Time Limit</div></div></div>`:''} 
${isFailed?`<div style="padding:0 24px 20px"><div style="padding:12px 16px;background:var(--rd-bg);border:1px solid rgba(248,113,113,.12);border-radius:var(--r2);font-size:.86rem"><strong style="color:var(--rd)">Breach:</strong> <span style="color:var(--t2)">${c.breach_reason||'Rule violation'}</span></div><button class="btn btn-primary btn-sm" style="margin-top:12px" onclick="navigate('buy')">Retry Challenge &rarr;</button></div>`:''}${isPassed?`<div style="padding:0 24px 20px"><div style="padding:12px 16px;background:var(--gr-bg);border:1px solid rgba(52,211,153,.12);border-radius:var(--r2);font-size:.86rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px"><span style="color:var(--gr);font-weight:600">&#10004; Evaluation Passed</span><button class="btn btn-outline btn-sm" onclick="showCertificate('pass',{name:'${(user?.first_name||'')+' '+(user?.last_name||'')}',size:'${F(c.account_size)}',type:'${c.challenge_type==='two_step'?'2-Step':'1-Step'}',date:'${c.passed_at?new Date(c.passed_at).toLocaleDateString():new Date(c.created_at).toLocaleDateString()}',id:'${c.id.slice(0,8).toUpperCase()}'})">View Certificate</button></div></div>`:''}</div>`}).join('')}`;};
...
<div class="eval-tabs" style="width:100%;margin-bottom:20px">${tb('overview','Payouts')}${tb('users','Users')}${tb('codes','Promo Codes')}${tb('ctrader','cTrader')}${tb('log','Audit Log')}</div>
...
${adminTab==='ctrader'?`<div id="ctraderBox">Loading cTrader status...</div>`:''}
...
if(adminTab==='ctrader'){
  try{
    const st=await api('/api/admin/ctrader/status');
    ...
    $('ctraderBox').innerHTML='<div class="card" style="padding:32px;text-align:center;color:var(--rd)">Failed to load cTrader status</div>';
  }
}
