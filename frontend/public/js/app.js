...
// Landing pricing — click opens register, remembers plan
function selectPlan(size){
  selectedPlan={size:size,type:currentEval};
  showAuth('register');
}
...
// Mobile menu toggle
function toggleMobile(){const sb=document.querySelector('.sidebar');sb.classList.toggle('mobile-open')}
...
window.render_dashboard=async function(){
  $('page-dashboard').innerHTML=LOADING;
  try{const s=await api('/api/dashboard/stats');
  // Show pending payment challenges
  ...
  ${pendingChallenges.length?`<div class="card" style="border-color:var(--am);background:var(--am-bg)"><div class="card-title" style="color:var(--am)">&#9888; Pending Payment</div>
  ${pendingChallenges.map(c=>`<div class="row"><span class="row-label">${F(c.account_size)} ${c.challenge_type==='two_step'?'2-Step':'1-Step'} Challenge</span><span class="row-value" style="color:var(--am)">Awaiting crypto payment</span></div>`).join('')}
  <p style="color:var(--t2);font-size:.82rem;margin-top:10px">Complete your crypto payment to activate your challenge. Check your email for the payment link.</p></div>`:''}

  ${activeChallenges.length?`<div class="card"><div class="card-title">Active Accounts</div>
  ...
  }catch(e){$('page-dashboard').innerHTML='<div class="card"><div class="empty">Failed to load dashboard.</div></div>'}
};

// CHALLENGES
window.render_challenges=async function(){$('page-challenges').innerHTML=LOADING;const d=await api('/api/challenges');$('page-challenges').innerHTML=`
...
// FUNDED
window.render_funded=async function(){$('page-funded').innerHTML=LOADING;const d=await api('/api/funded');$('page-funded').innerHTML=`
  ...
  <button class="btn btn-primary btn-sm" style="margin-top:16px" onclick="showPayoutModal('${a.id}')">Request Payout</button></div>`).join('')||'<div class="card"><div class="empty">No funded accounts yet. Pass an evaluation to get funded.</div></div>'}`};
...
window.render_buy=async function(){$('page-buy').innerHTML=`
  ...
  <div class="card" style="margin-top:18px"><div class="card-title">Included With Every Plan</div>
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;color:var(--t2);font-size:.84rem">
    <div>&#10003; No time limit</div><div>&#10003; No minimum trading days</div><div>&#10003; Static drawdown</div>
    <div>&#10003; EAs and copy trading</div><div>&#10003; Hedging permitted</div><div>&#10003; cTrader &amp; MetaTrader 5</div>
    <div>&#10003; Crypto &amp; bank payouts</div><div>&#10003; Scale up to $2M</div><div>&#10003; 24/7 support</div>
  </div></div>`;renderPricing(PLANS[currentEval],$('buyGrid'),'purchase')};
...
async function purchase(s){
  ...
  if(!confirm('Purchase '+F(s)+' '+( currentEval==='two_step'?'2-Step':'1-Step')+' challenge?\n\nYou will be redirected to pay with crypto.'))return;
  try{
...
// PAYOUT MODAL — replaces prompt()
function showPayoutModal(fundedId){
  const modal=document.createElement('div');
  modal.className='modal-bg';
  modal.id='payoutModal';
  modal.innerHTML=`<div class="modal">
    <button class="modal-close" onclick="document.getElementById('payoutModal').remove()">&times;</button>
    <h2>Request Payout</h2>
    <p>Choose your preferred payout method</p>
    <div style="display:flex;flex-direction:column;gap:10px">
      <button class="btn btn-primary btn-full" onclick="submitPayout('${fundedId}','crypto_usdt')">USDT (TRC-20)</button>
      <button class="btn btn-primary btn-full" onclick="submitPayout('${fundedId}','crypto_usdc')">USDC (ERC-20)</button>
      <button class="btn btn-outline btn-full" onclick="submitPayout('${fundedId}','bank_transfer')">Bank Transfer</button>
    </div>
  </div>`;
...
// PAYOUTS
window.render_payouts=async function(){$('page-payouts').innerHTML=LOADING;const d=await api('/api/payouts');const paid=d.filter(p=>p.status==='paid').reduce((s,p)=>s+p.trader_amount,0);const pending=d.filter(p=>['requested','approved','processing'].includes(p.status)).reduce((s,p)=>s+p.trader_amount,0);$('page-payouts').innerHTML=`
...
// TRADES
window.render_trades=async function(){$('page-trades').innerHTML=LOADING;const d=await api('/api/trades');$('page-trades').innerHTML=`
...
// PROFILE
window.render_profile=async function(){$('page-profile').innerHTML=LOADING;const u=await api('/api/users/profile');$('page-profile').innerHTML=`
  ...
  <button type="submit" class="btn btn-primary" style="margin-top:22px">Save Changes</button></form></div>`};
...
// ADMIN
window.render_admin=async function(){$('page-admin').innerHTML=LOADING;try{const o=await api('/api/admin/overview');const u=await api('/api/admin/users');const p=await api('/api/admin/payouts');$('page-admin').innerHTML=`
  ...
  <div class="stats">
    ...
    <div class="stat s-green"><div class="stat-label">Active</div><div class="stat-value">${o.active_challenges}</div></div>
    <div class="stat s-blue"><div class="stat-label">Funded</div><div class="stat-value">${o.total_funded}</div></div>
    <div class="stat s-red"><div class="stat-label">Pending</div><div class="stat-value">${o.pending_payouts}</div></div>
  </div>
...
// Handle return from crypto payment
