...
router.get('/', authenticate, (req, res) => {
  const payouts = queryAll(`SELECT * FROM payouts WHERE user_id='${req.user.id}' ORDER BY requested_at DESC`);
  res.json(payouts);
});
...
router.post('/request', authenticate, (req, res) => {
  try {
    ...
    const acct = queryOne(`SELECT * FROM funded_accounts WHERE id='${sanitize(funded_account_id)}' AND user_id='${req.user.id}'`);
    ...
    const pending = queryOne(`SELECT COUNT(*) as c FROM payouts WHERE funded_account_id='${funded_account_id}' AND status IN ('requested','under_review','approved','processing')`);
    ...
    const id = generateId();
    run(`INSERT INTO payouts (id, user_id, funded_account_id, gross_profit, split_pct, trader_amount, firm_amount, payout_method, wallet_address)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
