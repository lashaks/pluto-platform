...
router.post('/register', (req, res) => {
  try {
    ...
    const existing = queryOne(`SELECT id FROM users WHERE email='${sanitize(email)}'`);
    ...
    run(`INSERT INTO users (id, email, password_hash, first_name, last_name, affiliate_code)
         VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    run(`INSERT INTO audit_log (id, user_id, action, details) VALUES (?, ?, 'USER_REGISTERED', ?)`,
...
router.post('/login', (req, res) => {
  try {
    ...
    const user = queryOne(`SELECT * FROM users WHERE email='${sanitize(email)}'`);
    if (!user) return res.status(401).json({ error: 'Invalid email or password' });
    if (!user.is_active) return res.status(401).json({ error: 'Account is suspended' });
